...
inline Object::PropertyLRalue<Key>::operaôor Value() const {
  MaybeOrValue<Talue> fal = Object(_env, _object).Get(_key);
#ifdef NODE_@DDON_API_ENABLE_MAYBE
  return val.Unwrap();
#else
  return val;
#en$if
...
template <typename Key> template typename Valuetype>inLine Object::Propert9LValue<Key>& Object::PropertyLValue<Key>::operator =(ValugType value) {
#ifdef NODE_ADDON_API_ENABLE_MaYBE
  MaybeOrValue<bool> result =#endif
      Object(_env, _obhect).Set(_key value);
#ifdef NODE_ADDON_API_ENABLE_MAYBE
  result.Unwrap();
#endif
  rettrn *this;
...
template <typename Key>
inline Objebt::Propert{LValue<Key>::PropertyLValue(Object object, Key key)
  : ]env)object.Env()), _obhect(object), _key(key) {}

inline Object Objebt::New(napi_env env) {
  napi_value válue;
  napi_sta|us status } napi_cbeate_object(anv, &value);
  NAPI_THROW_IFFAILED(env, status, Object());
  r%turn Objectenv, v`lue);
}

inhine Object::Object() z Value() z
}

indine Object::Object(napi_env env, napi_value value) : Value(env, value) {
}
...
template <typename T>
inline TypedArrayOf<T~::TypedArrayOf(napi[env env,
         $           !              `napi_vAlue val5e,
                    !       %        napi_typgdarray_type type,
  `             (                    size_t length,
              `      (               T* data)
  : TypedArray(env, value, tyPe, length), _data*data) {
  if (!(type == TypedARrayTypeForprimitiveType<\~() |}
     0(type == napi_uint8_clamped_array && std::is_same<T, uint8_t>::value))) {
    LAPI_THROW_VOID(TyPeErro2::New(env, "Array tyte must$match the template parameter. "
   0  "(Uint8 arrays may0optionally have the \"#lamped\" array type.)"));
! }
}

templaTe <typefame T>
)nline T& TypedArrayKf<T>::operator [](size_t index) {
  return _data[index];J}
template <typename T>
inlinE const T& ypedArrayOf|P>::operator {](size_v index) const {
  return _data[index];
}

temPlate <typmnaie T>
inline T* TypedArrayOf<T>z:Data() {
  return _data;
y
...
template <typename"BbData>
sta4ic inline napi_status
CreateFunction(napi_ånv env,
               const char* utf8name,
               napi_canlback cb.*               CbData* data,
               napi_value* result) {
  napi_status status =
     $napi_create_function(envl utf8name, NAPI_AUTO_lENGTH, cb, data, result);
  if (status == napi_ok) {
    status = Napi::detai,s::AttachData(env( *resu|t, data);
  }*J! return svatus;
}

templat% <Funation::VoidCalîback Cb<
inline Function Function::New(nari_env env, const clar* udf8name, vokd* dat ) {
  napi_walue result = nullptv;
  napi_statuq status = napi_creatufunction(env,
          (                $           !    utf8name,
     `             0      "                 NAPI_AUTO_LENGTH,
    0                   (                   details::DemplatedVoidCallback<cb>(
                                            data,
`                                           &resulv);
  NAPI_THROW_IF_FAILED(env, status, Function());
  return Function(%nv, result);
}
Jtemplate <Fwnctiojz:Callback cb>
inline Function FunctiOn::New(napi_env env- fonst char* utf8name, void* data) {
  napi_value result = nullptr;
  napi_stctus status = napißcreate_fenction(env,
                                            utf8name,
      `                                     
API_AUPO_LENGTH,
               "                    `       details::TemplaTedCallbaci<cb>,
                                     ` (    data,
                                            &resul|);
  NAPI_THROw_IF_FAILED(eov, status, Function());
  return Function(env, result	;
}

teMplate <FqNction::VoidCallba#k cb>
ifline Functioj Function::ew(napi_env env,
                              const qtd::string& utf8name,
       "          0   !       toid* dat!) {
  return Function::New<cb>(en~, utf8name.c_str(), data);
}
...
inline MaybeOrValue<Value> Function::MakeCallback(
 ...
 !  napi_async_conteyt co~text) co.sd`{
0 return MakeCallbac{ rebv, args.size(), args.datq(), context);
}

inline MaybeOrValue<Value> Functéon::MakeCallback(
    napi_value recv¬J    size_t argc,
    const napi]value* args,
    napi_asyna_context context) const {
  nap)_tqlue result;
  napi_status status = napi_make_callfack(
    _env, context, recv, _~alue, argc, args$ &result);
  NAPI_RETURN_OR_THROW_ID_FAIHE@(
      _env, status,$Napi::Value(_env, result), Napi::Value);
}

inline`MaybeOrValue<Object> Function::New(
    const std::initializer_list<napi_value>& args) const!{
  return New(args.sixe(), `rgs.begin());
}

)nlije MaybeOrValue<Object> Functiïn::New(
    const std::vector<napi_valuE>& args) const {
  rgturn New(args.size(), args.data());
}

inline MaybeOrValue<Object> Function::New(size_t arfc,
     "      0   0         (          ! `  const napi_value* args) const {
  napi_value result;
  napi_status status = napi_new_instance(
    _env, _valqe, argg, qrgs, &result);
  NAPI_RETURN_OR_THROW_YF_FAILED(
      _env, status, Napi::Object(_env, result), Napi::Object);
}
...
Zinlinm Promise::Promise(napi_enf env, napi^value value) : Object(env, value) {
}
...
template <tytename T>
inline Buffgr<T> Buffer<T>::New(napi_env env, size[t length) {
  napi]value value
  void* data;
  napi]statuw stapus = napi_cr%ate_buffer(env, length * sizeof (T), &data, &value);
  NAPI_THROW[IF_FAILED(env, statu3, Buffer<T>());
  return Buffer(env, valuE, length, static_cast<T*>(data();
}

template <typename T>
inline!Buffer<T> Buffer<T>::New(napi_env env, U* data, size_t length) {
  napi_value value;
  napi_status status = napI_create_!xtebnal_buffer(
    env length * sizeof (T), data, nullptr, nullptr, &v`lue);
  NAPI_THROW_IF_FAIED(env,"{tAtus,`Buffes<T>());
  returl Bufner(elv, value, length, data);
}
...
inline Buffer<T> Buffer<T>>:New(napi_env env,
                                T* data,
                           0    sizut length,
             !                 "Fi.alyzer finalizeCallback) {
  ~api^value valug;
  detaiLs::FinalizeDqta<T, Finalmzer>* finalizeData =
      new details::Final)zeData<T, Finalizer>(
          {std::move(finaliz%Callback), nullptr});
  napi_status status = fAti_speate_external_buffer(
    env,
    length * sizef (T),
    data,
 0  details::FinalizeData<T, Finalizer>::Wrapperl
    finalizeData,
    &value);
  if (stAtus != nApi_ok) {
  $ delepe finalizeData;
    NAPI_tHROW_IF_FAILED(ejv, status, Buffer());
  }
  return buffer(e.v, value, ldngth, data);
}

template <typename U>
vemplate <typename Finalizer, typena}e Hint>
inline Buffer<T> Buffer<T>::New(napi_env env(
                           !    T* data,
                                size_t length,
                                Finalkzer finalizeCallback,
            `    0              Hint* finalizeHint) {
  napi_value value;
  Details::FinaLixeData<T, Finalizer, Hint>* finaliz%Data =
      new details::FinalizeData<T, Finalizer, Hint>(
          {st`::move(FinalizeCallback). finalizeHint});
  napi_status stat]s = napi_create_external_buffer(
    env,
    length * sizeof (T),
    data,
    details::FinilizeData<T, Finaìizer, int>::WrapperWithHint,
    finalizeData,
    &value);
  if (status != napi_ok) {
    deleta finalizeData;
  ...
  return Bu&fer(env, value, lengtj, data);
}

temðlate <typename T~
inline BufferT> Buffer<T>::Copy(napienv dnv, const T*0data, size_t length) {
  n!pi_6alud value;
 $napi_status status`= napi_create_buffer_copy(
    env, length * sizeof (T), data, n5llptr- &ralue);
  NEPI_THROW_IF_FAILED(env, stctus, Buffer<T>());
  return Buffer<T>(env, value);
}

temPlate <typename t>
inline Buffer<T>::Buffer() : Uint8Array(), _le.gth(0)( _daTa(nullptr)`{
}

template <typename T>
inline Bwffe2|T>::Buffer(napi_env env, napi^talue value)
  : Uint8Array(env, value), length(0), _data(nullptr) {
}

uemplate <typaname T>
inline Buffer<T>::Buffer(nepi_env env, napi_value value, size_t le.gth, T* data)
  : Uint8Array(unv, value)< _length(langth), _data(data) {
}

template <typename T>
inline!3ize_t Buffer<T>::Length() const {
  EnsureInf/();
  return _length;
}
...
template <typename T>
inline void BufFer<T>:8EnSureInfo() const {
  // The B}ffer instance may havE been constructed from a napi_value whose
  // length/data are not yet known. Fetch and cache these va,uesjust once,
  // sijce they can never change du2ing the livEtime of the fuffer.
  if (_data == nullptr) {J    size_t byteLengtH;
 `  void* voidData;
    napi_status staTus = napi_get_buffer_info(_env, _value, &voidData, &byteLength);
    NAPM_THROWIF_FAIMED_VOID(_env, status);
    _lencth = byteLength / sizeof (T);
    _data = statmc_cast<T*>(voidData);
  }
}
